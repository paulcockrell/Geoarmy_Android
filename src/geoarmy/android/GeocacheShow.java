...
public class GeocacheShow extends Activity {
	...
	private static final CharSequence REMOVE_FAVORITE  = " Remove favourite";
	TextView geocacheId, geocacheName, geocacheLat, geocacheLon, geocacheNotes,
			geocacheOwner;
	TextView geocacheStatus, geocacheGeoType, geocacheTerrain,
			geocacheDifficulty, geocacheSize;
	...
	private void drawScreen(final location geocache) {
		...
		this.geocacheDifficulty.setText("Difficulty: "
	...
	private void setButtonListeners() {
		foundButton.setOnClickListener(new Button.OnClickListener() {
			public void onClick(View v) {
				try {
					declareGeocacheFound(v.getContext(), thisGeocache.getId(),
							thisGeocache.getFound());
				} catch (Exception e) {
		...
		favoriteButton.setOnClickListener(new Button.OnClickListener() {
			public void onClick(View v) {
				try {
					declareGeocacheFavorite(v.getContext(), thisGeocache
							.getId(), thisGeocache.getFavorite());
				} catch (Exception e) {
	...
	public void setButtonText() {
		if (thisGeocache.getFound()) {
			...
			foundButton.setCompoundDrawablesWithIntrinsicBounds(d, null, null,
					null);
		} else if (!thisGeocache.getFound()) {
			foundButton.setText(DECLARE_FOUND);
			Drawable d = getResources().getDrawable(R.drawable.star_add);
			foundButton.setCompoundDrawablesWithIntrinsicBounds(d, null, null,
					null);
		}
		if (thisGeocache.getFavorite()) {
			favoriteButton.setText(REMOVE_FAVORITE);
			Drawable d = getResources().getDrawable(R.drawable.heart_delete);
			favoriteButton.setCompoundDrawablesWithIntrinsicBounds(d, null,
					null, null);
		} else if (!thisGeocache.getFavorite()) {
			favoriteButton.setText(DECLARE_FAVORITE);
			Drawable d = getResources().getDrawable(R.drawable.heart_add);
			favoriteButton.setCompoundDrawablesWithIntrinsicBounds(d, null,
					null, null);
		}
	}
	...
	public void onNetworkError(String error) {
		...
		MessageTools.alert("Sorry a network error has occured",
				GeocacheShow.this);
	}
